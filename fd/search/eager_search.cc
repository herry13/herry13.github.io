...
static SearchEngine *_parse_astar(OptionParser &parser) {
    ...
    if (!parser.dry_run()) {
        ...
        opts._set("open", open);
        opts._set("f_eval", f_eval);
        opts._set("reopen_closed", true);
        engine = new EagerSearch(opts);
    }
...
static SearchEngine *_parse_greedy(OptionParser &parser) {
    ...
    if (!parser.dry_run()) {
        ...
        opts._set("open", open);
        opts._set("reopen_closed", false);
        opts._set("pathmax", false);
        opts._set("mpd", false);
        ScalarEvaluator *sep = 0;
        opts._set("f_eval", sep);
        opts._set("preferred", preferred_list);
        engine = new EagerSearch(opts);
    }
    return engine;
}
